...
function refreshSettingsPanel() {
    ...
    const componentThemes = getComponentThemes();
    ...
    document.querySelectorAll('[data-component-setting]').forEach(select => {
        const type = select.getAttribute('data-component-setting');
        select.value = componentThemes[type] || select.value;
    });
    document.querySelectorAll('[data-component-value]').forEach(value => {
        const type = value.getAttribute('data-component-value');
        const option = COMPONENT_OPTIONS[type]?.find(item => item.id === componentThemes[type]);
        value.textContent = option?.label || '未配置';
    });
...
function setupSettingsPanel() {
    ...
    document.querySelectorAll('[data-component-setting]').forEach(select => {
        select.addEventListener('change', () => {
            const type = select.getAttribute('data-component-setting');
            setComponentTheme(type, select.value);
            refreshSettingsPanel();
            logTerminal(`[设置] 已切换${select.closest('.component-setting-row')?.querySelector('.settings-toggle-text')?.textContent || type}：${select.options[select.selectedIndex]?.text}`);
        });
    });
    ...
    window.addEventListener('componentThemeChanged', () => {
        applyComponentThemes(getComponentThemes());
        refreshSettingsPanel();
    });
}
